...
class ExpenseScreen extends StatelessWidget {
      ...
      body: Obx(() {
          ...
          return Center(
            child: Column(
              ...
              children: [
                ...
                SizedBox(height: 2.h),
                Text('Loading expenses...', style: TextStyle(fontSize: 15.sp, color: kSubText)),
              ],
            ),
          );
        }
        return Column(
          children: [
            ...
            Expanded(child: _buildExpensesList(controller)),
          ],
        );
      }),
  ...
  PreferredSizeWidget _buildAppBar(ExpenseController controller) {
    return AppBar(
      title: Text('Expenses', style: TextStyle(fontSize: 15.sp, fontWeight: FontWeight.w800, color: Colors.white)),
  ...
  Widget _buildSummaryCards(ExpenseController controller) {
    return Obx(() => Container(
      ...
      child: SingleChildScrollView(
        ...
        child: Row(
          children: [
            _buildSummaryCard('Total Expense', controller.formatAmount(controller.totalExpense.value), kDanger, Icons.trending_down, 28.w),
            SizedBox(width: 2.w),
        ...
        children: [
          Row(
            children: [
              ...
              Expanded(child: Text(title, style: TextStyle(fontSize: 13.sp, color: kSubText, fontWeight: FontWeight.w500))),
            ],
          ),
          SizedBox(height: 1.h),
          Text(isNumber ? amount : amount, style: TextStyle(fontSize: 15.sp, fontWeight: FontWeight.w800, color: color)),
        ],
      ),
    );
  }

  Widget _buildFilterBar(ExpenseController controller) {
    return Container(
      width: 100.w,
              ...
              Expanded(
                ...
                child: Container(
                  ...
                  child: TextField(
                    ...
                    style: TextStyle(fontSize: 15.sp, color: kText),
                    decoration: InputDecoration(
                      hintText: 'Search by number, vendor, description...',
                  ...
                  child: Obx(() => DropdownButtonHideUnderline(
                    child: DropdownButton<String>(
                      ...
                      items: controller.filterOptions.map((filter) {
                        return DropdownMenuItem(value: filter, child: Text(filter, style: TextStyle(fontSize: 15.sp, color: kText)));
                      }).toList(),
                      onChanged: (value) {
            ...
            child: Row(
              children: controller.expenseTypes.map((type) {
                ...
                return Padding(
                  ...
                  child: FilterChip(
                    label: Text(type, style: TextStyle(fontSize: 13.sp)),
                    selected: isSelected,
                    onSelected: (selected) {
                    ...
                    labelStyle: TextStyle(fontSize: 13.sp, color: isSelected ? kPrimary : kSubText),
                  ),
                );
              }).toList(),
            ),
                    ...
                    Row(
                      children: [
                        ...
                        Text(
                          ...
                          style: TextStyle(fontSize: 13.sp, color: kPrimary),
                        ),
                      ],
                    ),
                    GestureDetector(
  ...
  Widget _buildExpensesList(ExpenseController controller) {
    if (controller.expenses.isEmpty) {
      return Center(
        child: Column(
          mainAxisAlignment: MainAxisAlignment.center,
          children: [
            Icon(Icons.trending_down, size: 15.w, color: kSubText.withOpacity(0.5)),
            SizedBox(height: 2.h),
            Text('No expense records found', style: TextStyle(fontSize: 15.sp, color: kSubText)),
            SizedBox(height: 2.h),
            ElevatedButton(
    ...
    return NotificationListener<ScrollNotification>(
      ...
      child: ListView.builder(
        ...
        itemCount: controller.expenses.length + (controller.hasMore.value ? 1 : 0),
        itemBuilder: (context, index) {
          if (index == controller.expenses.length) {
            return _buildLoadingMoreIndicator();
          }
          final expense = controller.expenses[index];
          return _buildExpenseCard(controller, expense);
        },
      ),
    );
  }
  ...
  Widget _buildExpenseCard(ExpenseController controller, Expense expense) {
              ...
              children: [
                Row(
                  ...
                  children: [
                    Container(
                      ...
                      child: Icon(controller.getTypeIcon(expense.expenseType), size: 7.w, color: typeColor),
                    ),
                    SizedBox(width: 3.w),
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          Row(
                            children: [
                              Text(expense.expenseNumber, style: TextStyle(fontSize: 15.sp, fontWeight: FontWeight.w800, color: kText)),
                              SizedBox(width: 2.w),
                              Container(
                                padding: EdgeInsets.symmetric(horizontal: 2.w, vertical: 0.3.h),
                                decoration: BoxDecoration(color: statusColor.withOpacity(0.1), borderRadius: BorderRadius.circular(8)),
                                child: Text(expense.status, style: TextStyle(fontSize: 13.sp, color: statusColor, fontWeight: FontWeight.w600)),
                              ),
                            ],
                          ),
                          SizedBox(height: 0.5.h),
                          Text(expense.expenseType, style: TextStyle(fontSize: 13.sp, color: typeColor, fontWeight: FontWeight.w600)),
                          if (expense.vendorName.isNotEmpty)
                            Text(expense.vendorName, style: TextStyle(fontSize: 13.sp, color: kSubText)),
                        ],
                      ),
                    ),
                    Column(
                      ...
                      children: [
                        Text(controller.formatAmount(expense.totalAmount), style: TextStyle(fontSize: 15.sp, fontWeight: FontWeight.w800, color: kDanger)),
                        Text(DateFormat('dd MMM yyyy').format(expense.date), style: TextStyle(fontSize: 13.sp, color: kSubText)),
                      ],
                    ),
                  ],
                ),
                ...
                Row(
                  children: [
                    if (expense.status == 'Draft')
                      Expanded(
                        child: OutlinedButton.icon(
                          ...
                          label: Text('Post', style: TextStyle(fontSize: 13.sp, color: kSuccess)),
                    ...
                    if (expense.status == 'Draft') SizedBox(width: 2.w),
                    Expanded(
                      child: OutlinedButton.icon(
                        onPressed: () => _showExpenseDetails(expense),
                        icon: Icon(Icons.visibility, size: 4.w, color: kPrimary),
                        label: Text('Details', style: TextStyle(fontSize: 13.sp, color: kPrimary)),
          ...
          child: StatefulBuilder(
            builder: (context, setState) {
              ...
              return Column(
                ...
                children: [
                  Text('Add Expense', style: TextStyle(fontSize: 15.sp, fontWeight: FontWeight.w800, color: kText)),
                  SizedBox(height: 2.h),
                  Expanded(
                    child: SingleChildScrollView(
                      child: Form(
                        ...
                        child: Column(
                          children: [
                            ...
                            SizedBox(height: 2.h),
                            _buildDropdown('Expense Type', expenseType, controller.expenseTypes.skip(1).toList(), (v) => setState(() => expenseType = v!)),
                            SizedBox(height: 2.h),
                            _buildVendorDropdown(selectedVendorId, (v) => setState(() => selectedVendorId = v), controller.vendors),
                            SizedBox(height: 2.h),
                            ...
                            Container(
                              ...
                              child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                                Text('Total Amount', style: TextStyle(fontSize: 15.sp, fontWeight: FontWeight.w700, color: kText)),
                                Text(controller.formatAmount(calculateTotal()), style: TextStyle(fontSize: 15.sp, fontWeight: FontWeight.w800, color: kDanger)),
                              ]),
                            ),
                          ],
                        ),
                  ...
                  Row(
                    children: [
                      Expanded(child: OutlinedButton(onPressed: () => Get.back(), child: Text('Cancel', style: TextStyle(fontSize: 15.sp)))),
                      SizedBox(width: 3.w),
                    ...
                    color: kPrimary,
                    size: 10.w,
                  )) : Text('Save Expense', style: TextStyle(fontSize: 15.sp, color: Colors.white)),
                      ))),
                    ],
                  ),
                ],
        ...
        child: Column(
          children: [
            ...
            Align(
              ...
              child: Text('Amount: ${_formatAmount((item['quantity'] * item['unitPrice']).toDouble())}', style: TextStyle(fontSize: 13.sp, color: kPrimary)),
            ),
          ],
        ),
      );
      ...
      Container(
        ...
        child: Column(
          ...
          children: [
            Row(
              children: [
                Container(width: 14.w, height: 14.w, decoration: BoxDecoration(color: kDanger.withOpacity(0.1), borderRadius: BorderRadius.circular(14)), child: Icon(Icons.trending_down, size: 7.w, color: kDanger)),
                SizedBox(width: 3.w),
                Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  Text(expense.expenseNumber, style: TextStyle(fontSize: 15.sp, fontWeight: FontWeight.w800, color: kText)),
                  Text(DateFormat('dd MMM yyyy').format(expense.date), style: TextStyle(fontSize: 13.sp, color: kSubText)),
                ])),
                Container(padding: EdgeInsets.symmetric(horizontal: 2.w, vertical: 0.3.h), decoration: BoxDecoration(color: expense.status == 'Posted' ? kSuccess.withOpacity(0.1) : kWarning.withOpacity(0.1), borderRadius: BorderRadius.circular(8)), child: Text(expense.status, style: TextStyle(fontSize: 13.sp, color: expense.status == 'Posted' ? kSuccess : kWarning))),
              ],
            ),
            SizedBox(height: 2.h),
            _buildDetailRow('Expense Type', expense.expenseType),
            if (expense.vendorName.isNotEmpty) _buildDetailRow('Vendor', expense.vendorName),
            _buildDetailRow('Subtotal', _formatAmount(expense.subtotal)),
            if (expense.taxRate > 0) _buildDetailRow('Tax (${expense.taxRate}%)', _formatAmount(expense.taxAmount)),
            _buildDetailRow('Total Amount', _formatAmount(expense.totalAmount)),
            _buildDetailRow('Payment Method', expense.paymentMethod),
            if (expense.reference.isNotEmpty) _buildDetailRow('Reference', expense.reference),
            if (expense.description.isNotEmpty) _buildDetailRow('Description', expense.description),
            if (expense.items.isNotEmpty) ...[
              SizedBox(height: 2.h),
              Text('Items', style: TextStyle(fontSize: 15.sp, fontWeight: FontWeight.w700, color: kText)),
              SizedBox(height: 1.h),
              ...expense.items.map((item) => Container(
                ...
                child: Row(
                  children: [
                    Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      Text(item.description, style: TextStyle(fontSize: 13.sp, fontWeight: FontWeight.w600, color: kText)),
                      Text('${item.quantity} x ${_formatAmount(item.unitPrice)}', style: TextStyle(fontSize: 13.sp, color: kSubText)),
                    ])),
                    Text(_formatAmount(item.amount), style: TextStyle(fontSize: 15.sp, fontWeight: FontWeight.w700, color: kDanger)),
                  ],
                ),
              )).toList(),
            ],
            ...
            if (expense.status == 'Draft')
              ElevatedButton.icon(
                onPressed: () { Get.back(); Get.find<ExpenseController>().postExpense(expense.id); },
                icon: Icon(Icons.check_circle, size: 4.5.w),
                label: Text('Post Expense', style: TextStyle(fontSize: 13.sp)),
  ...
  void _showFilterDialog(ExpenseController controller) {
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        title: Text('Filter Expenses', style: TextStyle(fontSize: 15.sp, fontWeight: FontWeight.w700)),
        content: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            ListTile(
              leading: Icon(Icons.calendar_today),
              title: Text('Date Range', style: TextStyle(fontSize: 15.sp)),
            ...
            ListTile(
              ...
              title: Text('Clear Filters', style: TextStyle(fontSize: 15.sp)),
              onTap: () {
        ...
        actions: [TextButton(onPressed: () => Navigator.pop(context), child: Text('Close', style: TextStyle(fontSize: 13.sp)))],
      ),
    );
  }
  
  Widget _buildFAB(ExpenseController controller) {
  ...
  Widget _buildDropdown(String label, String value, List<String> items, Function(String?) onChanged) {
    return Container(
      ...
      child: DropdownButtonFormField<String>(
        ...
        style: TextStyle(fontSize: 15.sp, color: kText),
        dropdownColor: kCardBg,
        items: items.map((item) => DropdownMenuItem(value: item, child: Text(item, style: TextStyle(fontSize: 15.sp, color: kText)))).toList(),
        onChanged: onChanged,
      ),
    );
  }
      ...
      child: Container(
        ...
        child: Row(
          children: [
            ...
            SizedBox(width: 3.w),
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text(label, style: TextStyle(fontSize: 13.sp, color: kSubText)),
              Text(DateFormat('dd MMM yyyy').format(date), style: TextStyle(fontSize: 15.sp, fontWeight: FontWeight.w600, color: kText)),
            ])),
          ],
        ),
      ),
  ...
  Widget _buildVendorDropdown(String? selectedId, Function(String?) onChanged, List<Map<String, dynamic>> vendors) {
    ...
    return Container(
      ...
      child: DropdownButtonFormField<String>(
        ...
        style: TextStyle(fontSize: 15.sp, color: kText),
        dropdownColor: kCardBg,
        hint: Text('Select vendor', style: TextStyle(fontSize: 13.sp, color: kSubText)),
        items: vendors.map((vendor) => DropdownMenuItem(value: vendor['_id'].toString(), child: Text(vendor['name'], style: TextStyle(fontSize: 15.sp, color: kText)))).toList(),
        onChanged: onChanged,
      ),
    );
  }
  ...
  Widget _buildBankAccountDropdown(String? selectedId, Function(String?) onChanged, List<Map<String, dynamic>> bankAccounts) {
    ...
    return Container(
      ...
      child: DropdownButtonFormField<String>(
        ...
        style: TextStyle(fontSize: 15.sp, color: kText),
        dropdownColor: kCardBg,
        hint: Text('Select bank account', style: TextStyle(fontSize: 13.sp, color: kSubText)),
        items: bankAccounts.map((acc) => DropdownMenuItem(value: acc['_id'].toString(), child: Text(acc['accountName'], style: TextStyle(fontSize: 15.sp, color: kText)))).toList(),
        onChanged: onChanged,
      ),
    );
  }
  ...
  Widget _buildDetailRow(String label, String value) {
    return Padding(
      ...
      child: Row(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          SizedBox(width: 30.w, child: Text(label, style: TextStyle(fontSize: 13.sp, color: kSubText, fontWeight: FontWeight.w500))),
          Expanded(child: Text(value, style: TextStyle(fontSize: 15.sp, color: kText, fontWeight: FontWeight.w600))),
        ],
      ),
    );
  }
