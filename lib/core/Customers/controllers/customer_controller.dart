...
class CustomerController extends GetxController {
  ...
  Future<void> createCustomer(Map<String, dynamic> customerData) async {
    try {
      ...
      if (response.statusCode == 201) {
        ...
        final data = jsonDecode(response.body);
        AppSnackbar.error(kDanger, 'Error', data['message'] ?? 'Failed to add customer');
      }
    } catch (e) {
  ...
  Future<void> updateCustomer(String id, Map<String, dynamic> customerData) async {
    try {
      ...
      if (response.statusCode == 200) {
        AppSnackbar.success(kSuccess, 'Success', 'Customer updated successfully');
        await fetchCustomers();
      } else {
        final data = jsonDecode(response.body);
        AppSnackbar.error(kDanger, 'Error', data['message'] ?? 'Failed to update customer');
      }
    } catch (e) {
  ...
  Future<void> deleteCustomer(String id, String name) async {
    try {
      ...
      if (response.statusCode == 200) {
        AppSnackbar.success(kSuccess, 'Success', 'Customer "$name" deleted successfully');
        await fetchCustomers();
      } else {
        final data = jsonDecode(response.body);
        AppSnackbar.error(kDanger, 'Error', data['message'] ?? 'Failed to delete customer');
      }
    } catch (e) {
  ...
  Future<void> exportToPdf() async {
    try {
      ...
      if (kIsWeb) {
        ...
        AppSnackbar.success(kSuccess, 'Success', '${customers.length} customers exported to PDF');
      } else {
  ...
  Future<void> exportToExcel() async {
    try {
      ...
      if (kIsWeb) {
        ...
        AppSnackbar.success(kSuccess, 'Success', '${customers.length} customers exported to Excel');
      } else {
